---
description: How Kredo Protocol Works - Authorization-Based Virtual Card System
---
...
```mermaid
architecture-beta
    group kredo_cloud(cloud)[Kredo Protocol Cloud]
    
    service dashboard(server)[Dashboard] in kredo_cloud
    service mobile(server)[Mobile App] in kredo_cloud
    service api(server)[API Gateway] in kredo_cloud
    
    group blockchain(cloud)[Blockchain Layer]
    
    service zk_verifier(server)[ZK Proof Verifier] in blockchain
    service intent_engine(server)[Intent Execution] in blockchain
    service smart_contracts(database)[Smart Contracts] in blockchain
    
    group liquidity(cloud)[Liquidity Layer]
    
    service fog_pool(database)[Liquidity Fog Pool] in liquidity
    service policy_db(database)[Authorization Policies] in liquidity
    service pool_storage(disk)[Pool Storage] in liquidity
    
    group settlement(cloud)[Settlement Layer]
    
    service crypto_settlement(server)[Crypto Settlement] in settlement
    service visa_api(server)[Visa API] in settlement
    service merchant_network(internet)[Merchant Network] in settlement
    
    dashboard:R --> L:api
    mobile:R --> L:api
    api:B --> T:zk_verifier
    zk_verifier:R --> L:intent_engine
    intent_engine:B --> T:smart_contracts
    smart_contracts:R --> L:fog_pool
    fog_pool:T --> B:policy_db
    pool_storage:T --> B:fog_pool
    intent_engine:B --> T:crypto_settlement
    intent_engine:B --> T:visa_api
    crypto_settlement:R --> L:merchant_network
    visa_api:R --> L:merchant_network
```
